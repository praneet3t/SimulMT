...
def load_model(model_path, dtype_str="bf16"):
    """
    Load model + tokenizer.  Supports two checkpoint layouts:

      1. PEFT / LoRA adapter directory  (has adapter_config.json)
         → load base model from adapter_config["base_model_name_or_path"],
           then wrap with PeftModel.from_pretrained.

      2. Merged / plain HF model directory
         → load directly with AutoModelForCausalLM.

    Layout (1) is what `train.py` produces: the HF hub push only contains
    the LoRA weights + lm_head, NOT the merged weights.  Loading with
    AutoModelForCausalLM in that case silently ignores the adapter weights
    and runs the *untrained* base model — which produces garbage.
    """
    ...
    # ------------------------------------------------------------------ #
    # Detect whether model_path is a PEFT adapter directory              #
    # ------------------------------------------------------------------ #
    # Works for local paths AND HF hub ids (hub_download returns local)
    is_peft = False
    base_model_path = model_path
    try:
        from huggingface_hub import hf_hub_download
        cfg_file = hf_hub_download(model_path, "adapter_config.json")
        with open(cfg_file) as fh:
            adapter_cfg = json.load(fh)
        base_model_path = adapter_cfg.get("base_model_name_or_path", model_path)
        is_peft = True
        print(f"  Detected PEFT adapter. Base model: {base_model_path}")
    except Exception:
        # local path: check directly
        local_cfg = os.path.join(model_path, "adapter_config.json")
        if os.path.exists(local_cfg):
            with open(local_cfg) as fh:
                adapter_cfg = json.load(fh)
            base_model_path = adapter_cfg.get("base_model_name_or_path", model_path)
            is_peft = True
            print(f"  Detected PEFT adapter (local). Base model: {base_model_path}")

    # ------------------------------------------------------------------ #
    # Load base model                                                     #
    # ------------------------------------------------------------------ #
    print(f"  Loading base model in {dtype_str} ...")
    ...
    # ------------------------------------------------------------------ #
    # Apply PEFT adapter (if applicable)                                  #
    # ------------------------------------------------------------------ #
    if is_peft:
        ...
        model = model.merge_and_unload()   # fuses LoRA + lm_head; no runtime overhead
